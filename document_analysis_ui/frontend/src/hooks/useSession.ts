...
import type { Session, CreateSessionResponse, UploadResponse } from '../services/types';

interface UseSessionReturn {
  session: Session | null;
  isLoading: boolean;
  error: string | null;
  createSession: (financialThreshold?: number) => Promise<CreateSessionResponse>;
...
export function useSession(): UseSessionReturn {
  ...
  const createSession = useCallback(async (financialThreshold: number = 15000) => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await api.createSession(financialThreshold);
  ...
  const uploadFiles = useCallback(async (files: File[]) => {
    ...
    setError(null);
    try {
      const response = await api.uploadDocuments(session.id, files);
